...
from src.DES.TripleDESCipher import TripleDESCipher
from src.utils.constants import CipherMode, PaddingMode


class TripleDESDemo:
    def __init__(self):
        self.key_16 = bytes([0x13, 0x34, 0x57, 0x79, 0x9B, 0xBC, 0xDF, 0xF1,
                             0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF])
        self.key_24 = bytes([0x13, 0x34, 0x57, 0x79, 0x9B, 0xBC, 0xDF, 0xF1,
                             0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF,
                             0xFE, 0xDC, 0xBA, 0x98, 0x76, 0x54, 0x32, 0x10])
        self.iv = bytes([0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF])
        self.results_dir = Path('tripledes_demo_results')
    ...
    def generate_test_files(self):
        ...
        with open(test_dir / 'text.txt', 'w', encoding='utf-8') as f:
            f.write('Hello, TripleDES! Тестирование шифрования TripleDES алгоритма с различными режимами.\n' * 10)
    ...
    async def test_mode(self, data: bytes, mode: CipherMode, name: str, key: bytes, key_name: str):
    ...
    async def run_demo(self):
        ...
        print("=" * 80)
        print("ДЕМОНСТРАЦИЯ TRIPLE DES АЛГОРИТМА")
        print("=" * 80)
        print()
        ...
        keys = [
            (self.key_16, '16 bytes (Keying Option 2)'),
            (self.key_24, '24 bytes (Keying Option 1)')
        ]
        ...
        for filename, description in test_files:
            ...
            for key, key_name in keys:
                print(f"\n  Ключ: {key_name}")
                for mode in modes:
                    result = await self.test_mode(data, mode, filename, key, key_name)
                    ...
                    if result['success']:
                        encrypted_path = self.results_dir / f'TripleDES_{filename}_{key_name.replace(" ", "_")}_{mode.name}.enc'
        ...
        for key, key_name in keys:
            ...
            if key_results:
                key_success = sum(1 for r in key_results if r['success'])
                print(f"  {key_name}: {key_success}/{len(key_results)} успешных тестов")
